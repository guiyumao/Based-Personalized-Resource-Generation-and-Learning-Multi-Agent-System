...
> 来源: https://www.runoob.com/mysql/mysql-index.html
> 爬取时间: 2026-06-25 13:29:17
> 学科: databases | 难度: 进阶
> 标签: 数据库, 索引, B-Tree, 进阶
...
我们可以使用ALTER TABLE命令可以在已有的表中创建索引。
...
我们可以在创建表的时候，你可以在CREATE TABLE语句中直接指定索引，以创建表和索引的组合。
...
在 MySQL 中，你可以使用CREATE UNIQUE INDEX语句来创建唯一索引。
...
以下是一个使用ALTER TABLE命令创建唯一索引的实例：假设我们有一个名为 employees 的表，包含 id 和 email 列，现在我们想在 email 列上创建一个唯一索引，以确保每个员工的电子邮件地址都是唯一的。
...
我们也可以在创建表的同时，你可以在CREATE TABLE语句中使用UNIQUE关键字来创建唯一索引。
...
请注意，使用UNIQUE关键字后，索引名称将自动生成，你也可以根据需要指定索引名称。
