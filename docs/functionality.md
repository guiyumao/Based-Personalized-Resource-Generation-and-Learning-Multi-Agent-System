...
- 学生端已经支持学习路径、课件阅读、课件多版本切换、在线答题、即时反馈、错题本、重练题、学习报告和智能答疑。
...
- 课件区支持多课件版本选择，当前可在 `interactive`、`concise`、`case` 三类候选课件间切换。
- 课件生成失败时会自动降级到本地快速版课件，不阻塞页面。
