...
- 提供智能答疑与学习分析接口，可同时输出学生可读讲解与系统可写入的结构化分析结果。
- 提供知识图谱可视化节点和边数据。
- 课件生成已升级为“真实教师授课风格”输出，会结合知识底稿、RAG 检索上下文和学习者画像生成完整 Markdown 课件。
- 习题生成已升级为“真实命题教师”输出，每次请求都会实时生成题目，不使用前端本地缓存。
...
| Method | Path | Handler | Description |
| --- | --- | --- | --- |
| `POST` | `/qa/analyze` | `analyze_question` | Answer one student question and return structured learning analysis. |
...
- 学生端已接入智能答疑与学习分析，可提交学习问题并查看自然语言讲解、知识漏洞识别、路线更新建议和资源推荐。
- 学生端“生成学习课件”和“生成课后自测”均为实时请求后端，每次点击都会重新生成，不使用浏览器本地缓存。
- 学生端课件和自测题已支持“双阶段生成”：先立即展示快速版内容，再在后台继续尝试升级为远程正式版，避免长时间空白等待。
- 当远程课件生成超时或失败时，学生端会自动切换到快速版本地课件，避免页面长时间卡在生成进度上。
- 学生端题目作答为单题单次提交模式：同一题提交后立即锁定，只能查看标准答案与解析，不能再次作答。
- 学生端已增加课件生成进度、自测题生成进度，以及“习题生成请求/响应”“答疑分析请求/响应”调试面板，便于直接确认后端是否返回了结果。
