...
import { evaluationApi, type ApiEnvelope, type ExerciseGenerationPayload, type ExerciseGenerationResponse, type ExerciseItem, type PracticeFeedback, type PracticeSubmissionPayload } from '../../api'
...
const exerciseForm = reactive<ExerciseGenerationPayload>({
  ...
  learner_profile: { learning_style: 'visual', mastery: 62 }, exercise_count: 5,
...
async function generateExercises() {
  if (!exerciseForm.knowledge_point.trim()) { ElMessage.warning('请先输入知识点'); return }
...
<template>
  <div>
    ...
    <div style="display:flex;gap:10px;align-items:center;margin-bottom:20px;flex-wrap:wrap">
      <input v-model="exerciseForm.knowledge_point" placeholder="输入知识点/主题"
        style="flex:1;min-width:160px;padding:10px 14px;border-radius:12px;border:1px solid var(--line);background:var(--bg);color:var(--text);font-size:14px;outline:none;font-family:inherit" />
    ...
    <div v-else-if="!genError" style="text-align:center;padding:60px;color:var(--muted);border-radius:18px;background:var(--panel);border:1px solid var(--line)">输入知识点，点击"生成练习题"开始自测</div>
  </div>
</template>
