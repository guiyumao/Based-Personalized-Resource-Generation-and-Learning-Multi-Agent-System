...
const dashboardCards = computed<DashboardCard[]>(() => {
  ...
  const cards: DashboardCard[] = []
    ...
    // Exercise-derived cards — only show when there's real activity
  ...
  <div>
    ...
    <router-link
      ...
      style="display:flex;align-items:center;gap:16px;padding:20px 24px;
        ...
        margin-bottom:20px;text-decoration:none;color:inherit;cursor:pointer;
    ...
    >
      <div style="width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px;
        background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 15%,transparent),color-mix(in srgb,var(--accent-deep) 15%,transparent))">
        🧠
      </div>
      <div style="flex:1">
        <div style="font-weight:700;font-size:16px">用户画像分析</div>
        <div style="font-size:13px;color:var(--muted);margin-top:2px">查看 AI 实时分析的您的学习状态</div>
      </div>
