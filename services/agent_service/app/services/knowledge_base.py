...
class KnowledgeBaseService:
    ...
    def __init__(self) -> None:
        self._articles: list[tuple[tuple[str, ...], KnowledgeArticle]] = [
            ...
            (
                ("高数", "高等数学", "微积分", "calculus", "极限", "导数", "积分"),
                KnowledgeArticle(
                    title="高等数学：极限、导数与积分",
                    subject="高等数学",
                    level="大学基础",
                    summary=(
                        "高等数学围绕函数变化规律展开，核心主线通常是极限、连续、导数、微分、积分以及它们之间的联系。"
                        "学习时既要会判定概念和条件，也要能把结论放到具体函数、图像和应用场景中检查。"
                    ),
                    concepts=[
                        "极限描述自变量逼近某一点或无穷远时函数值的变化趋势，做题时要先分清是点极限、单侧极限还是无穷极限。",
                        "函数连续通常要同时检查函数值存在、极限存在且二者相等，分段函数尤其容易在连接点出错。",
                        "导数反映函数在某一点的瞬时变化率，也可以理解为曲线切线的斜率。",
                        "利用一阶导数可以分析单调区间和极值，利用二阶导数可以分析凹凸性和拐点。",
                        "定积分既可以理解为累积量，也可以理解为带符号面积，解题时要明确积分区间和被积函数性质。",
                    ],
                    syntax=[
                        "lim_{x -> a} f(x)",
                    ...
                    examples=[
                        "判断分段函数在连接点是否连续时，先分别求左右极限，再比较函数值。",
                        "求函数极值时，先求导并找驻点，再结合导数符号表判断单调性变化。",
                        "求定积分应用题时，先确定积分表达式，再检查区间方向和几何意义。",
                    ],
                    mistakes=[
                        "把极限存在误当成函数在该点一定有定义，没有检查函数值。",
                        "求导时只记公式，不检查复合函数、乘积或商的结构，容易漏掉链式法则。",
                        "只找到驻点就直接下结论，没有结合导数符号或实际区间判断极值。",
                        "定积分换元后忘记同步变换积分上下限，或者忽略函数符号导致面积判断错误。",
                    ],
                    applications=[
                        "用导数分析成本、收益、速度、增长率等变化问题。",
                        "用定积分表示路程、面积、累计量或平均值问题。",
                        "用函数单调性和凹凸性解释图像变化趋势。",
                    ],
                    checks=[
                        "我能不能说清楚连续需要检查哪三个条件？",
                        "我能不能区分导数公式、导数符号和导数应用这三个层次？",
                        "我能不能在求积分前先说明区间、符号和几何意义？",
                    ],
                ),
            ),
        ]
