...
def get_profile_status(user_id: int, db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    profile = db.get(UserProfile, user_id)
    if profile is None:
        return {"completed": False, "skipped": False, "dimensions_filled": 0, "total_dimensions": len(PROFILE_DIMENSION_KEYS)}
    ...
    filled = sum(1 for k in PROFILE_DIMENSION_KEYS if dimensions.get(k))
    skipped = habits.get("profile_skipped", False)

    return {
        "completed": filled >= 3,  # at least 3 dimensions filled = usable profile
        "skipped": bool(skipped),
        "dimensions_filled": filled,
        "total_dimensions": len(PROFILE_DIMENSION_KEYS),
        "dimensions": {k: bool(dimensions.get(k)) for k in PROFILE_DIMENSION_KEYS},
    }
...
def skip_profile_setup(user_id: int, db: Session = Depends(get_db)) -> dict[str, object]:
    ...
    profile = db.get(UserProfile, user_id)
    if profile is None:
        profile = UserProfile(user_id=user_id, mastery_json={}, learning_style="VARK", cognitive_abilities={}, habits={})
        db.add(profile)

    habits = profile.habits if isinstance(profile.habits, dict) else {}
    profile.habits = {**habits, "profile_skipped": True}
    db.commit()
    return {"skipped": True, "message": "Profile setup skipped. You can return to it later from the workspace."}
